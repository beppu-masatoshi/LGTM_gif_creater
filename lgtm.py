...
def process_gif(
    ...
    for frame in ImageSequence.Iterator(im):
        ...
        stamped = draw_lgtm(frame, text, color, outline, x_ratio, y_ratio) if show_text else frame.convert("RGBA")
        # GIF保存用にパレット変換(透過を維持)
...
def main():
    ...
    parser.add_argument("--color", default="white", help="文字の塗り色 (デフォルト: white)")
    parser.add_argument("--outline", default="black", help="文字の縁取り色 (デフォルト: black)")
    parser.add_argument(
    ...
    n = process_gif(
        ...
        x_ratio, y_ratio, args.start, args.end,
    )
